...
void contextSwitch(struct PCB *curr_proc, struct PCB *next_proc,int curr_time)
{
    //stop current
    //change it's state
    if (curr_proc)
    {
        ...
        curr_proc->laststop=curr_time;
        printpcb(curr_proc,curr_time);
    }
    //run next
    //change it's state
    ...
    if(next_proc->state ==-1)
    {
        ...
        (next_proc->waiting_time) += curr_time - (next_proc->arrival_time);
    }
    else{
        next_proc->state = RESUMED;
        (next_proc->waiting_time) += curr_time - (next_proc->laststop);
    }
    printpcb(next_proc,curr_time);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        {
        case FCFS:
            if((ready_processes->count == 0) && !curr_proc)
             idle_waiting++;
            if (curr_proc)
            {
            ...
            {
                contextSwitch(curr_proc, ready_processes->head->data,curr_time);
                curr_proc = ready_processes->head->data;
                 curr_proc->state=STARTED;
                curr_proc->waiting_time=(curr_time)-(curr_proc->arrival_time);                
                 printpcb(curr_proc,curr_time);
            }
            break;
        case SJF:
           ...
           if((ready_processes->count == 0) && !curr_proc)
            ...
            if (curr_proc)
            {
                ...
                if (curr_proc->remaining_time == 0) //finished
                {
                    ...
                    finished_process(curr_proc);
                    curr_proc->finish_time=curr_time;
                    curr_proc->waiting_time=curr_time-(curr_proc->arrival_time)-(curr_proc->run_time);
                    printpcb(curr_proc,curr_time);
                    curr_proc = NULL;
                }
            }
            ...
            if (next && (!curr_proc || ((struct PCB *)next->data)->remaining_time < curr_proc->remaining_time))
            {
                ...
                contextSwitch(curr_proc, (struct PCB *)next->data,curr_time);
